...
public class VentanaQuijoteConColaDeHilos extends JFrame {
	private static final long MSGS_REFRESCO = 5; // Refresco entre scroll suave y siguiente scroll suave
	...
	// ATENCIÓN!!!! Podría haber algún problema de acceso concurrente (sería mala suerte, pero... esas cosas pasan.
	// Mejor Vector que es una estructura sincronizada (no puede haber dos threads a la vez manipulándola)
	private Vector<Thread> hilosActivos = new Vector<>();
			...
			public void run() {
				...
				if (pixelsVertical>0) {
					for (int i=0; i<pixelsVertical; i++) {
						...
						try {Thread.sleep(MSGS_REFRESCO); } catch (InterruptedException e) {}
					}
				} else {
					// for (int i=0; i>pixelsVertical; i--) {
				...
				hilosActivos.remove(0); // me quito del final de la cola
			}
		};
		hiloActual.start();
	}
